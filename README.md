...
Fork of neofetch, with more logos.

insert-image-or-delete-this-line

Note: This project is not finished at all, and should not be used.
...
Run "fylfetch" command, hidden in somewhere. Should be in path.
...
A: Use `hyfetch -c` //can be removed, no need

## Q: When I use this in my MotD, no art displays. How do I get the art back?

A: Most likely, the `stdout` detection is set to auto and is removing the ascii art and colors. To change this, you can: set `args` in your hyfetch.json to `"--stdout=off"`, add `--stdout=off` to the `neowofetch` command in your motd script, or set the option in $HOME/.config/neofetch/config.conf to off.

# Contributing

To make changes to our codebase, you first need to create a fork.
[How to create a fork](https://raw.githubusercontent.com/EggToaster/fylfetch/master/how-to-create-a-fork.wav) // Note: this is hardcoded, be careful if you want to fork this as different project.

If they work correctly, you can commit and push these changes using git command or Github Desktop. // or vscode, or any ides that support git, or git gui, or literaly github website itself, we will not care about what you use

Then, you can create a pull request on Github so that it can (hopefully) go into our next release!

You can also install your version locally by running `pip install .` in the repo root.

# TODO

* [ ] (Important!) remove hyfetch, many renames
* [ ] Lua rewrite

# IMPORTANT
do not look below here, still unedited
